"""
Problem Set 3 - The 6.0001 Word Game
# Created by: Kevin Luu <luuk> and Jenna Wiens <jwiens>
This game is a lot like Scrabble or Words With Friends. 
Letters are dealt to players, who then construct one or more words 
using their letters. Each valid word earns the user points, 
...
def get_word_score(word, n):
    ...
    second_component = max(1, (7*wordlen - 3*(n-wordlen))) #Return the largest number
    
                                                        #OR> second_component = 7*wordlen - 3*(n-wordlen)
...
def deal_hand(n):
    ...
    for i in range(num_vowels - 1): #  -1: replace one of the vowels slot for a wildcard
...
def is_valid_word(word, hand, word_list):
    ...
    """
    word = word.lower()
    #When wildcard is not used
    if '*' not in word:
        if word not in word_list:  
            return False
        
        word_in_dict = get_frequency_dict(word)  # word letter(key) -> no.of times its repeated
        for letter in word_in_dict.keys():
            if word_in_dict[letter] > hand.get(letter, 0):  # The number of times a letter is required exceeds the number provided by the hand
                return False
        return True
        
    #Consider use of wildcard
    elif word.count('*') == 1:
        for vowel in VOWELS:
            temp_word_flag = True
            temp_word = word.replace('*', vowel)
            if temp_word not in word_list:
                continue #used to end the current iteration in a for loop (or a while loop), and continues to the next iteration.
            
            word_in_dict = get_frequency_dict(temp_word)
            temp_hand = hand.copy()
            temp_hand.pop('*')
            temp_hand[vowel] = temp_hand.get(vowel, 0) + 1
            for letter in word_in_dict.keys():
                if word_in_dict[letter] > temp_hand.get(letter, 0):
                    temp_word_flag = False
                    break
            if temp_word_flag: 
                return True
            else:
    ...
    while calculate_handlen(hand) > 0:# As long as there are still letters left in the hand:
        ...
        if word == '!!':# If the input is two exclamation points:
            ...
            if is_valid_word(word, hand, word_list) == True:# If the word is valid:
                ...
                total_score += get_word_score(word, n)# and the updated total score
            ...
            n = calculate_handlen(hand)#update hand size
        print()
    
    # Game is over (user entered '!!' or ran out of letters)
    if word == '!!':
        print('Total points for this hand:', total_score)# so tell user the total score
    elif calculate_handlen(hand) <= 0:       
        print('Ran out of letters.', 'Total points for this hand:', total_score)# so tell user the total score
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    """
    #user input
    initial_hands  = int(input('Enter total number of hands: '))
    #flags
    replayed_flag = False
    substituted_flag = False
    #initial variable values
    game_hands = initial_hands
    game_score = 0 
    
    for i in range(game_hands):
        hand = deal_hand(HAND_SIZE)            
        print("Current Hand:", end= " ")
        display_hand(hand)
        
        if substituted_flag == False:
            subst_call = input('Would you like to substitute a letter? ')
            if subst_call.lower() == 'yes':

...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        """
        self.width = int(width)# TODO:写报告注意这里int了不然519行报错
...
# 以下为Problem 6的简答题，以下回答与报告文档中的内容完全相同。
# Question 1
# show_plot_compare_strategies('Time to clean 80% of a 20x20 room, for various numbers of robots','Number of robots','Time / steps')
"""
答：
无论是StandardRobot还是FaultyRobot，完成相同的清理任务
所需要的时间都随着机器人数量的增加而减少。
同样是清理一个20×20房间的80%，在机器人数量相同的情况下，
StandardRobot完成任务需要的时间略少于FaultyRobot，且二
者之差随着机器人数量的增加而缩小。
"""
...
"""
答：
在房间面积相同但形状不同时，完成清理所需要的时间与房间形状
的长宽比（width / height）有关，根据图像Figure 2可以看出，
对于一个面积为300的房间，清理完它的80%所需要的的时间在形状
为20×15（比例1.33）时最少，当长宽比增大或减小时，清理时间
都会增加。
无论房间形状如何，StandardRobot完成任务的时间都要小于
FaultyRobot。
"""